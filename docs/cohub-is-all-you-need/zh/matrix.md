...
| `cohub.bp.work-kit-product` | 用 Work Kit 做真 App 产品 | Apps, SDK, Scopes | `cohub-work-kit`, `cohub-work-publish` | intermediate | builder |
| `cohub.bp.minimal-scopes` | 以最小权限发布 App | appScopes, viewer grants | `cohub-apps` | intermediate | builder |
...
| `cohub.bp.work-commerce` | App 内售卖功能与积分 | Apps, Billing, SDK | `cohub-work-kit` | advanced | builder |
...
| `cohub.bp.port-preview` | 端口预览（开发向） | Apps (`port`), Sandbox ports | `cohub-apps` | intermediate | builder |
| `cohub.bp.hide-cohub-bar` | 隐藏公开 App 的 Cohub 底栏 | Apps, 计费权益 | `cohub` CLI | starter | builder |
| `cohub.bp.work-lifecycle` | 发布 / 版本 / 停用 / 可见性 | Apps | `cohub-apps` | intermediate | builder |
| `cohub.bp.viewer-auth-user-scopes` | 访客授权 + 账户级权限 | Apps, SDK | `cohub` | advanced | builder, agent-author |
...
| `cohub.bp.public-identity-slugs` | 用户名与 Space/App slug，拼公开 URL | Profile, Space, Apps | `cohub` | starter | builder |
...
| 给别人直接打开 | **App** | 发私有 Sandbox 链接 |
| 静态演示/站点 | App `directory` + `base: "./"` | 静态托管上用 History 路由 |
| Cohub 壳里的交互产品 | App + SDK + 最小权限 | 一上来全开权限 |
...
| 给别人直接打开 | **App** | 发私有 Sandbox 链接 |
| 在 Space 安装可复用工具 | **App Center** | 每个项目都复制一份 App URL |
| 查看 Space 用量 | **Space Activity** | 向所有观众暴露费用字段 |
| 找到最近的 Space 或 Chat | **Command palette** | 把全局热度列表当作个人上下文 |
| 静态演示/站点 | `directory` App + `base: "./"` | 静态托管上用 History 路由 |
| Cohub 壳里的交互产品 | App + SDK + 最小权限 | 一上来全开权限 |
...
| Cookbook | 结果 | 难度 | 角色 |
|----------|------|------|------|
| [weekend-static-work](./cookbooks/weekend-static-work.md) | 公开 directory App | starter | builder |
...
| [paid-work-minimum](./cookbooks/paid-work-minimum.md) | 可商业化 App 壳 | advanced | builder |
