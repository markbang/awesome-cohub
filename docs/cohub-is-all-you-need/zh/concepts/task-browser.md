...
**Task Browser** 是专门浏览和查看生成任务及其他 Task Run 的多模态任务界面。它以仓库管理的 App 形式提供；Chat 内生成任务托盘移除后，它成为跟踪异步生成工作的主要入口。
...
创建生成的 App 通常需要 viewer grant `generation.create` 创建任务，并需要 `taskrun.view` 轮询或查看结果。创建任务不等于获得读取任务的权限。

## 运行时行为

- 浏览器按当前视图申请最小权限：Mine 使用账户级权限，Space/Session 使用对应 Space 的权限。
- `client.auth.requestSpace()` 让观众在一次授权流程中选择其他 Space；App 只知道被选中的 Space。
