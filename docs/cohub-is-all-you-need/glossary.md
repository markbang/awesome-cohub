...
| **App** | 应用（旧称 Work） | Published share unit (`file` / `directory` / `port`); canonical SDK/API term since v2.37 | [work](./concepts/work.md) |
...
| **Space Activity** | Space 活动 | Bounded usage, contributor, model, and App-view summary for one Space | [space-activity](./concepts/space-activity.md) |
| **Command palette** | 命令面板 | Personal-relevance navigation with Recent, cached defaults, and search | [command-palette](./concepts/command-palette.md) |
| **Prompt quick action** | Prompt 快捷操作 | Prompt-template button declared with `quick-action` frontmatter | [command-palette](./concepts/command-palette.md) |
...
| **Commerce** | 商业化 | Paid features/credits inside Apps | [commerce](./concepts/commerce.md) |
